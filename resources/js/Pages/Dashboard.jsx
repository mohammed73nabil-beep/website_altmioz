...
export default function Dashboard({ unreadMessagesCount, pendingRequestsCount, totalProjects, recentProjects = [], systemHealth }) {
    ...
    const StatCard = ({ icon, iconBgColor, iconColor, label, value, link, extra, extraColor, extraIcon }) => (
        <Link href={link} className="block bg-white dark:bg-[#1a1a1a] p-6 rounded-2xl border border-slate-200 dark:border-white/5 hover:border-[#C5A059]/50 transition-all duration-300 group cursor-pointer hover:shadow-lg hover:shadow-[#C5A059]/5">
            <div className="flex justify-between items-start mb-4">
                <div className={`p-2.5 ${iconBgColor} rounded-xl ${iconColor} group-hover:scale-110 group-hover:bg-[#C5A059]/10 group-hover:text-[#C5A059] transition-all duration-300`}>
                    <DynamicIcon name={icon} />
                </div>
                <div className="text-left">
                    <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">{label}</p>
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white mt-1 group-hover:text-[#C5A059] transition-colors">{value}</h3>
                </div>
            </div>
            {extra && (
            ...
            <div className="space-y-4">
                <div className="p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02]">
                    <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-[#C5A059] text-sm font-bold">{health?.uptime}</span>
                    </div>
                    ...
                    <div className="w-full bg-slate-200 dark:bg-[#2a2a2a] h-1.5 rounded-full mt-3 overflow-hidden">
                        <div className="bg-[#C5A059] h-1.5 rounded-full w-[99%]"></div>
                    </div>
                </div>
                <div className="p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02]">
                    <div className="flex justify-between items-center mb-1">
                        ...
                        {health?.dbConnected ? (
                            <span className="text-primary text-[11px] font-bold px-2 py-0.5 bg-primary/10 rounded">متصل</span>
                        ) : (
                ...
                <div className="p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02]">
                    <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-[#C5A059] text-sm font-bold">{health?.completionRate}%</span>
                    </div>
                    <p className="text-[11px] text-slate-500">مقارنة بالمشاريع المسجلة</p>
                    <div className="w-full bg-slate-200 dark:bg-[#2a2a2a] h-1.5 rounded-full mt-3 overflow-hidden">
                        <div className="bg-[#C5A059] h-1.5 rounded-full transition-all duration-1000" style={{ width: `${health?.completionRate || 0}%` }}></div>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        ...
        >
            ...
            {/* Welcome Banner */}
            <div className="bg-gradient-to-l from-[#C5A059] to-[#b38615] rounded-2xl p-8 relative overflow-hidden shadow-lg shadow-[#C5A059]/20">
                <div className="relative z-10">
                    <h2 className="text-2xl font-bold text-white mb-2">نظرة عامة على النظام</h2>
                    <p className="text-white/80 max-w-xl">مرحباً بك مجدداً، إليك آخر التحديثات لمشاريع الديكورات.</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-slate-200 dark:border-white/5 p-6">
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-200 dark:border-white/5 flex items-start gap-3">
                        <DynamicIcon name="check_circle" className="text-primary text-[18px]" />
                        <div>
