...
export default function DesignService({ projects = [], galleryImages = [] }) {
    ...
    const steps = [
        { title: getContent('services.design.process.step1.title', 'معاينة الموقع'), icon: 'location_on', desc: getContent('services.design.process.step1.desc', 'زيارة الموقع لتقييم المساحة ومعرفة متطلبات التصميم والاحتياجات.') },
        { title: getContent('services.design.process.step2.title', 'تقديم المقترح'), icon: 'request_quote', desc: getContent('services.design.process.step2.desc', 'نقدم مقترح تصميمي شامل مع عرض سعر تفصيلي للموافقة عليه.') },
        { title: getContent('services.design.process.step3.title', 'التنفيذ والتركيب'), icon: 'construction', desc: getContent('services.design.process.step3.desc', 'استخدام أحدث الأدوات لتنفيذ التصميم بدقة وسرعة مع مراعاة الجودة.') },
        { title: getContent('services.design.process.step4.title', 'التسليم النهائي'), icon: 'verified', desc: getContent('services.design.process.step4.desc', 'تسليم الحديقة مع ضمان جودة العمل والتأكد من مطابقة التصميم المعتمد.') }
    ];
    ...
    const advantages = [
        ...
        { title: getContent('services.design.why.item2.title', 'كوادر هندسية'), icon: 'groups', desc: getContent('services.design.why.item2.desc', 'فريق من المهندسين الزراعيين ذوي الكفاءة العالية في أعمال اللاندسكيب.') },
    ...
    return (
        <div className="bg-[#f8f7f6] dark:bg-[#022C22] min-h-screen text-gray-800 dark:text-gray-100 font-display selection:bg-[#16A34A] selection:text-[#064E3B]" dir="rtl" lang="ar" style={{ fontFamily: '"Almarai", "Manrope", sans-serif' }}>
            <Head>
                <title>{`تصميم الحدائق - ${siteName}`}</title>
                <meta name="description" content={getContent('services.design.hero.desc', 'نقدم خدمة تصميم وتنسيق الحدائق المنزلية بأحدث التصاميم العصرية واللاندسكيب المبتكر في المملكة.')} />
            ...
            <section className="relative h-[65vh] min-h-[550px] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <img
                        ...
                        fetchpriority="high"
                    />
                    <div className="absolute inset-0 bg-[#064E3B]/30" style={{ opacity: getOpacity('services.design.hero.image', 30) / 100 }}></div>
                    <div className="absolute inset-0 hero-gradient" style={{ opacity: Math.min((getOpacity('services.design.hero.image', 30) + 40) / 100, 1) }}></div>
                </div>
                ...
                <div className="relative z-10 max-w-5xl mx-auto px-4 text-center mt-20">
                    ...
                    <h1 className="text-3xl md:text-5xl lg:text-7xl font-black text-white mb-6 leading-tight drop-shadow-xl reveal-on-scroll opacity-0 translate-y-10 transition-all duration-1000">
                        {getContent('services.design.hero.title', 'تصميم الحدائق')}
                    </h1>
                    <p className="text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto mb-10 leading-relaxed font-light reveal-on-scroll opacity-0 translate-y-10 transition-all duration-1000 delay-100">
                        {getContent('services.design.hero.subtitle', 'نقدم خدمات متكاملة لتصميم وتنسيق الحدائق وبناء الشلالات بأحدث المواصفات لتلبية كافة احتياجاتك.')}
                    </p>
            ...
            <GallerySection images={galleryImages} title="صور من أعمال تصميم الحدائق" />
            ...
            <section className="py-24 bg-white dark:bg-[#064E3B] relative z-20">
                <div className="max-w-7xl mx-auto px-4 text-center reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700">
                    <span className="text-[#16A34A] font-bold tracking-widest text-sm uppercase block mb-4">{getContent('services.design.services.badge', 'ماذا نقدم؟')}</span>
                    <h2 className="text-4xl font-black text-gray-900 dark:text-white mb-8">{getContent('services.design.services.title', 'خدمات تصميم الحدائق')}</h2>
            ...
            <section className="py-24 bg-[#064E3B] relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-4 relative z-10 text-center">
                    <span className="text-[#16A34A] font-bold tracking-widest text-sm uppercase block mb-4">{getContent('services.design.process.badge', 'آلية العمل')}</span>
                    <h2 className="text-4xl font-black text-white mb-16">{getContent('services.design.process.title', 'خطوات التصميم خطوة بخطوة')}</h2>
                ...
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex flex-col lg:flex-row items-center gap-16">
                        <div className="w-full lg:w-1/2 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700">
                            <span className="text-[#16A34A] font-bold tracking-widest text-sm uppercase block mb-4">{getContent('services.design.why.badge', 'لماذا نحن؟')}</span>
                        ...
                        <div className="w-full lg:w-1/2 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700 delay-200">
                            <div className="relative rounded-3xl overflow-hidden shadow-2xl h-[600px]">
                                ...
                                <div className="absolute bottom-10 right-10 left-10 text-white">
                                    ...
                                    <div className="text-xl text-[#16A34A]">في تصميم وتأهيل الحدائق</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto px-4">
                        <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-16 gap-6 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700">
                            <div>
                                ...
                                <h2 className="text-4xl font-black text-gray-900 dark:text-white">أحدث مشاريع تصميم الحدائق</h2>
                            </div>
            ...
            <section className="relative py-28 text-center bg-[#022C22] overflow-hidden">
                <div className="absolute inset-0 z-0">
                    ...
                    <div className="absolute inset-0 bg-[#064E3B]/40" style={{ opacity: getOpacity('services.design.hero.image', 40) / 100 }}></div>
                    <div className="absolute inset-0 hero-gradient" style={{ opacity: Math.min((getOpacity('services.design.hero.image', 40) + 40) / 100, 1) }}></div>
                </div>
