...
export default function BlogIndex({ posts, categories, currentCategory }) {
    ...
    const heroBg = pageContents?.['blog.hero.background_image'] 
        ? (pageContents['blog.hero.background_image'].startsWith('http') ? pageContents['blog.hero.background_image'] : `/storage/${pageContents['blog.hero.background_image']}`) 
        : "https://images.unsplash.com/photo-1499750310107-5fef28a66643?q=80&w=2070&auto=format&fit=crop";
    
    const heroOpacity = pageContentExtras?.['blog.hero.background_image'] ? parseInt(pageContentExtras['blog.hero.background_image']) : 80;
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark min-h-screen text-gray-800 dark:text-gray-100 font-display selection:bg-primary selection:text-slate-900" dir="rtl" lang="ar" style={{ fontFamily: '"Almarai", "Manrope", sans-serif' }}>
            <Head>
                ...
                <meta name="description" content="اقرأ أحدث المقالات والنصائح في مجال تصميم وديكورات داخلية، وتعرف على أحدث التطورات في مجال الديكورات." />

                {/* OpenGraph Tags */}
                <meta property="og:title" content={pageTitle} />
                <meta property="og:description" content="اقرأ أحدث المقالات والنصائح في مجال تصميم وديكورات داخلية." />
            ...
            <section className="relative h-[45vh] min-h-[400px] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <img
                        ...
                        src={heroBg}
                    />
                    <div className="absolute inset-0 bg-surface-dark" style={{ opacity: heroOpacity / 100 }}></div>
                    <div className="absolute inset-0 hero-gradient" style={{ opacity: Math.min((heroOpacity + 20) / 100, 1) }}></div>
                </div>

                <div className="relative z-10 max-w-5xl mx-auto px-4 text-center mt-10">
                    <span className="inline-block py-1.5 px-4 rounded-full border border-primary/30 text-primary font-bold text-sm tracking-widest uppercase mb-6 backdrop-blur shadow-sm">
                        مركز المعرفة
                    </span>
                    ...
                    <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed font-light reveal-on-scroll opacity-0 translate-y-10 transition-all duration-1000 delay-100">
                        اكتشف النصائح والأفكار المبتكرة في عالم الديكورات والزراعة الحديثة.
                    </p>
                </div>
            </section>
                    ...
                    {categories && categories.length > 0 && (
                        <div className="flex flex-wrap justify-center gap-3 mb-16 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700">
                            <Link
                                ...
                                className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all duration-300 shadow-sm ${!currentCategory ? 'decoration-gradient text-slate-900' : 'bg-white dark:bg-surface-dark text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 border border-gray-200 dark:border-white/10'}`}
                            >
                                الكل
                            </Link>
                            {categories.map((cat) => (
                                <Link
                                    ...
                                    className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all duration-300 shadow-sm ${currentCategory === cat.slug ? 'decoration-gradient text-slate-900' : 'bg-white dark:bg-surface-dark text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 border border-gray-200 dark:border-white/10'}`}
                                >
                                    {cat.name}
                                </Link>
                            ))}
                        ...
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {posts.data.map((post, idx) => (
                                    <Link key={post.id} href={route('blog.show', post.slug)} className="group bg-white dark:bg-surface-dark rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl border border-gray-100 dark:border-white/5 hover:border-primary/50 block transition-all duration-500 hover:-translate-y-2 reveal-on-scroll opacity-0 translate-y-10" style={{ transitionDelay: `${(idx % 3) * 100}ms` }}>
                                        <div className="h-56 overflow-hidden relative">
                                            ...
                                            />
                                            <div className="absolute inset-0 bg-gradient-to-t from-surface-dark/80 to-transparent opacity-80"></div>
                                            {post.category && (
                                                <div className="absolute top-4 right-4 bg-white/95 dark:bg-surface-dark/95 text-primary text-xs font-bold px-3 py-1.5 rounded-full shadow-sm">
                                                    {post.category.name}
                                                </div>
                                            )}
                                        </div>
                                        <div className="p-8">
                                            ...
                                            <h3 className="font-bold text-2xl text-gray-900 dark:text-white mb-4 line-clamp-2 leading-tight group-hover:text-primary transition-colors">{post.title}</h3>
                                            ...
                                            <div className="flex items-center justify-between mt-auto">
                                                <span className="text-sm font-bold text-primary flex items-center gap-2 group-hover:translate-x-1 transition-transform">
                                                    اقرأ المزيد
            ...
            <footer className="bg-[#040A12] py-8 text-center text-sm text-gray-500 border-t border-white/5 relative z-20">
                <p>نظام شركة مظلات التميز © {new Date().getFullYear()} — صُمِّم بواسطة <a href="https://wa.me/967781582995" target="_blank" rel="noopener noreferrer" className="text-primary font-bold hover:underline">شركة Aboras Soft</a></p>
            </footer>
        </div>
    );
}
