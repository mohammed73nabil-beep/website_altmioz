...
export default function ProjectsIndex({ projects = [] }) {
    ...
    const categories = ['الكل', 'صيانة مباني', 'كرفانات', 'بركسات', 'ترميم', 'عزل'];
    ...
    const filteredProjects = activeTab === 'الكل'
        ? projects
        : projects.filter(p => p.category === activeTab || (activeTab === 'كرفانات' && p.category?.includes('تجاري')));

    return (
        <div className="bg-[#f8f7f6] dark:bg-[#0B1F3A] min-h-screen text-gray-800 dark:text-gray-100 transition-colors duration-300" dir="rtl" lang="ar" style={{ fontFamily: '"Almarai", "Manrope", sans-serif' }}>
            <Head>
            ...
            <section className="relative h-[65vh] min-h-[500px] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <img
                        alt="High-end maintenance project background"
                        className="w-full h-full object-cover"
                        src={getImageUrl('projects.hero.background_image', "https://lh3.googleusercontent.com/aida-public/AB6AXuD0JAHsObzIwnKDCWmYTUyAsR9C7WINbtHiv7-SNPFkwpvyHrGM42q9H2J-Ee7DLoDv4ZlMqlXqVtRvDB8vhp8nYJohCzmTUqr5HcXUp2SHaYN4S_QfhWu6bX_c7gpYGuWPRa3TtWsyF62L3fbjnXlfn_Kh-HQJS1zAnRNrpSODl5zr5LVSSkKw1K7FuK46TOtZOwKzrUUQO90P_O0bee6_wDBb7dO-38mIlKKEpPBEQdmTbOAymBm0rvhuCFlMrx_V9LhK3fdjmCeF")}
                    />
                    <div className="absolute inset-0 bg-[#0B1F3A]" style={{ opacity: getOpacity('projects.hero.background_image', 60) / 100 }}></div>
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-4 text-center mt-20">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        {pageContents?.['projects.hero.primary_button_text'] && (
                            <a href={pageContents?.['projects.hero.primary_button_link'] || '#projects-grid'} className="gold-gradient hover:brightness-110 text-[#0B1F3A] font-bold py-4 px-10 rounded-lg transition-all duration-300 shadow-[0_0_20px_rgba(201,162,39,0.4)] hover:-translate-y-1">
                                {pageContents?.['projects.hero.primary_button_text']}
                            </a>
                        )}
                        {pageContents?.['projects.hero.secondary_button_text'] && (
                            <Link href={pageContents?.['projects.hero.secondary_button_link'] || route('contact')} className="border-2 border-white/30 hover:bg-white hover:text-[#0B1F3A] text-white font-bold py-4 px-10 rounded-lg transition-all duration-300 backdrop-blur-sm hover:-translate-y-1">
                                {pageContents?.['projects.hero.secondary_button_text']}
                            </Link>
                        )}
                    </div>
            ...
            {/* 2) FILTER SECTION */}
            <section id="projects-grid" className="py-8 bg-white dark:bg-[#071324] border-b border-gray-200 dark:border-white/5 sticky top-20 z-40 shadow-sm">
                <div className="max-w-7xl mx-auto px-4 overflow-x-auto no-scrollbar">
                    <div className="flex justify-start md:justify-center gap-2 md:gap-4 min-w-max pb-2">
                        {categories.map((cat, index) => {
                            ...
                            const filterKeys = ['all', 'maintenance', 'caravans', 'portacabins', 'renovation', 'insulation'];
            ...
            {/* 3) PROJECT GRID */}
            <section className="py-20 bg-[#f8f7f6] dark:bg-[#0B1F3A]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredProjects.length > 0 ? (
                            filteredProjects.map((project, index) => {
                                ...
                                return (
                                    <div key={project.id} className="project-card group relative bg-white dark:bg-[#071324] rounded-2xl overflow-hidden shadow-lg border border-gray-100 dark:border-white/5 transition-all duration-500 hover:shadow-[0_20px_40px_rgba(0,0,0,0.2)] hover:-translate-y-2 flex flex-col h-full">
                                        <div className="relative h-64 overflow-hidden">
                                            <img
                                                ...
                                                onError={(e) => { e.target.onerror = null; e.target.src = 'https://lh3.googleusercontent.com/aida-public/AB6AXuD0JAHsObzIwnKDCWmYTUyAsR9C7WINbtHiv7-SNPFkwpvyHrGM42q9H2J-Ee7DLoDv4ZlMqlXqVtRvDB8vhp8nYJohCzmTUqr5HcXUp2SHaYN4S_QfhWu6bX_c7gpYGuWPRa3TtWsyF62L3fbjnXlfn_Kh-HQJS1zAnRNrpSODl5zr5LVSSkKw1K7FuK46TOtZOwKzrUUQO90P_O0bee6_wDBb7dO-38mIlKKEpPBEQdmTbOAymBm0rvhuCFlMrx_V9LhK3fdjmCeF'; }} // Fallback if image missing
                                            />
                                            <div className="absolute top-4 right-4 bg-white/90 dark:bg-[#0B1F3A]/90 backdrop-blur text-xs font-bold px-3 py-1.5 rounded-full text-[#C9A227] shadow-sm">
                                                {pageContents?.[`${cmsKeyPrefix}.category`] || project.category}
                                            </div>
                                            {/* Hover Overlay */}
                                            <div className="project-overlay absolute inset-0 bg-gradient-to-t from-[#0B1F3A] via-[#0B1F3A]/80 to-transparent opacity-0 transition-opacity duration-300 flex flex-col justify-end p-6">
                                                ...
                                                <Link href={pageContents?.[`${cmsKeyPrefix}.details_button_link`] || route('our-projects.show', project.id)} className="inline-flex items-center gap-2 text-white font-bold hover:text-[#C9A227] transition-colors w-fit">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-4 mt-4 text-sm text-gray-500 dark:text-gray-400">
                                                    <div className="flex items-center gap-1">
                                                        <DynamicIcon name="location_on" className="text-[16px] text-[#C9A227]" />
                                                        <span>{pageContents?.[`${cmsKeyPrefix}.location`] || 'السعودية'}</span>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                        <DynamicIcon name="calendar_today" className="text-[16px] text-[#C9A227]" />
                                                        <span dir="ltr">{pageContents?.[`${cmsKeyPrefix}.year`] || new Date(project.created_at).getFullYear()}</span>
                                                    </div>
                                                </div>
                                            </div>

                                            <Link href={pageContents?.[`${cmsKeyPrefix}.details_button_link`] || route('our-projects.show', project.id)} className="mt-6 w-full flex justify-center py-3 text-center border border-[#C9A227]/30 text-[#C9A227] hover:bg-[#C9A227] hover:text-[#0B1F3A] font-bold rounded-lg transition-colors duration-300">
            ...
            <section className="py-20 bg-[#071324] border-y border-white/5 relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center divide-x divide-x-reverse divide-white/10">
                        <div className="p-4 flex flex-col items-center">
                            {pageContents?.['projects.stats.projects_completed.icon'] && (
                                <DynamicIcon name={pageContents['projects.stats.projects_completed.icon']} className="text-[#C9A227] text-4xl mb-2" />
                            )}
                            <div className="text-5xl md:text-6xl font-black text-transparent bg-clip-text gold-gradient mb-3" style={{ fontFamily: '"Manrope", sans-serif' }}>
                        ...
                        <div className="p-4 flex flex-col items-center">
                            {pageContents?.['projects.stats.years_experience.icon'] && (
                                <DynamicIcon name={pageContents['projects.stats.years_experience.icon']} className="text-[#C9A227] text-4xl mb-2" />
                            )}
                            <div className="text-5xl md:text-6xl font-black text-transparent bg-clip-text gold-gradient mb-3" style={{ fontFamily: '"Manrope", sans-serif' }}>
                        ...
                        <div className="p-4 flex flex-col items-center">
                            {pageContents?.['projects.stats.customer_satisfaction.icon'] && (
                                <DynamicIcon name={pageContents['projects.stats.customer_satisfaction.icon']} className="text-[#C9A227] text-4xl mb-2" />
                            )}
                            <div className="text-5xl md:text-6xl font-black text-transparent bg-clip-text gold-gradient mb-3" style={{ fontFamily: '"Manrope", sans-serif' }}>
                        ...
                        <div className="p-4 flex flex-col items-center">
                            {pageContents?.['projects.stats.engineers.icon'] && (
                                <DynamicIcon name={pageContents['projects.stats.engineers.icon']} className="text-[#C9A227] text-4xl mb-2" />
                            )}
                            <div className="text-5xl md:text-6xl font-black text-transparent bg-clip-text gold-gradient mb-3" style={{ fontFamily: '"Manrope", sans-serif' }}>
                                {pageContents?.['projects.stats.engineers.value'] || '50'}
                            </div>
                            <div className="text-white font-medium text-lg">
                                {pageContents?.['projects.stats.engineers.title'] || 'خبير فني مهندس'}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="max-w-7xl mx-auto px-4">
                    <div className="text-center mb-16">
                        {pageContents?.['projects.testimonials.badge'] && (
                            <span className="text-[#C9A227] font-bold tracking-widest text-sm uppercase block mb-3">{pageContents?.['projects.testimonials.badge']}</span>
                        )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {[1, 2, 3].map((index) => {
                            ...
                            return (
                                <div key={index} className="bg-gray-50 dark:bg-[#071324] p-8 rounded-2xl border border-gray-100 dark:border-white/5 relative">
                                    <DynamicIcon name="format_quote" className="absolute top-6 left-6 text-6xl text-[#C9A227]/10 rotate-180" />
                                    <div className="flex items-center gap-1 text-[#C9A227] mb-6">
                                    ...
                                    <div className="flex items-center gap-4 border-t border-gray-200 dark:border-white/10 pt-6">
                                        <div className="w-12 h-12 rounded-full bg-gray-200 dark:bg-white/10 flex items-center justify-center font-bold text-[#0B1F3A] dark:text-white">
                                            {name.charAt(0)}
                                        </div>
                                        <div>
            ...
            <section className="py-24 relative overflow-hidden bg-[#C9A227]">
                ...
                <div className="absolute inset-0 bg-gradient-to-l from-[#C9A227] to-[#e3c059]" style={{ opacity: pageContents?.['projects.cta.background_image'] ? getOpacity('projects.cta.background_image', 90) / 100 : 1 }}></div>
                ...
                <div className="max-w-5xl mx-auto px-4 relative z-10 text-center flex flex-col items-center">
                    {pageContents?.['projects.cta.icon'] && (
                        <div className="w-20 h-20 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center mb-6 text-[#0B1F3A]">
                            <DynamicIcon name={pageContents['projects.cta.icon']} className="text-4xl" />
                        </div>
                    )}
                    <h2 className="text-4xl md:text-5xl font-black text-[#0B1F3A] mb-6 leading-tight">
                    ...
                    <p className="text-[#0B1F3A]/80 text-xl font-medium mb-10 max-w-2xl">
                        {pageContents?.['projects.cta.description'] || 'تواصل معنا الآن للحصول على استشارة هندسية مجانية وعرض سعر مبدئي لتنفيذ متطلباتك بأعلى معايير الجودة.'}
                    </p>
                    <Link href={pageContents?.['projects.cta.button_link'] || route('contact')} className="bg-[#0B1F3A] hover:bg-white hover:text-[#0B1F3A] text-white font-bold text-xl py-5 px-14 rounded-xl transition-all duration-300 shadow-xl hover:shadow-[0_20px_40px_rgba(11,31,58,0.2)] transform hover:-translate-y-1 inline-flex items-center gap-3">
